...
maxsubser <- function(A, low, high) { # divide and conquer function, recursive
    if (high == low) { # when the length is 1
        ...
        if (left_sum >= right_sum && left_sum >= cross_sum) { # if left sum is greater than the right and cross sums
            return(c(left_low, left_high, left_sum)) # return the values of left sub
        } else { # close second if and open else
            if (right_sum >= left_sum && right_sum >= cross_sum) { # if right sum is the greatest
                return(c(right_low, right_high, right_sum)) # return the values of right sub
            } else { # close third if and open else
                return(c(cross_low, cross_high, cross_sum)) # then the greatest is the cross, so its values are returned
            } # close last else
        } # close the previous else
...
crossingsub <- function(A, low, mid, high) { # the max subarray which crosses the mid point, A is the vector; low, mid and high are indices on A
    ...
    for (i in mid:low) { # to start from mid and sum to low
        sum <- sum + A[i] # update sum with a new index to the left
        if (sum > left_sum) { # condition to update the max of left sum from the mid to low
    ...
    sum <- 0 # initial sum is zero
    for (j in (mid + 1):high) { # to start from mid+1 and sum to high
        sum <- sum + A[j]
        if (sum > right_sum) { # condition to update the max of right sum from the mid to low
