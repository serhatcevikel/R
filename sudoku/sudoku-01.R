...
multiple_clear_full_row <- function(sudoku01c, sudoku_sqc, sudoku_arc) { # find common item > 1 cells in rows/columns/3x3 and delete the common items from other cells 
    for (cap in 2:5) { # for 1 max size of common items
        for (row in 1:9) { # for 2 rows for control
            ...
            if (length(indices_vector) >= cap) { # if 1 size of cells at least the cap 
                ...
                for (comb in 1:n_comb) { # for 3 combinations
                    common_vec <- numeric(0) # initiate a vector to control the common items
...
indices_vector_max_size <- function(sudoku_arc,row,cap) { # get the column indices of vectors with non NA elements up to a size
    vector_indices <- numeric (0) # start an empty vector for the column indices of vectors
    vector_indices[1:9] <- NA # make the vector_indices empty of length 9
    sudoku_arc_row <- sudoku_arc[row,,] # sub vector of the row
    for (col in 1:9) { # 1 for each column
        ...
        size_of_nonna <- length(sudoku_arc_vector) # number of nonna items in the vector
        if(size_of_nonna >=2 && size_of_nonna <= cap) vector_indices[col] <- col # update the indices vector with vector with sizes below cap
    } # close for 1
    vector_indices <- vector_indices[is.na(sudoku_arc_vector)==FALSE] # nonna indices in the vector
